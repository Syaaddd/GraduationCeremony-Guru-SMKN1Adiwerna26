...
export default function InvitationPage({ teacher }) {
  ...
  const firstName = teacher.name.split(",")[0].replace(/^(Drs?\.|Dra\.|Dr\.)/i, "").trim().split(" ").slice(0,2).join(" ");
  ...
  return (
    ...
    >
      ...
      <div style={{ position:"fixed", inset:0, pointerEvents:"none", overflow:"hidden", zIndex:0 }}>
        ...
        <div style={{ position:"absolute", top:"40%", left:"-10%", width:"40vw", height:"40vw", borderRadius:"50%",
          background:"radial-gradient(circle, rgba(100,140,200,0.04) 0%, transparent 65%)", animation:"orb3 13s ease-in-out infinite" }}/>
      </div>

      {/* Sparkle dots */}
      {[...Array(12)].map((_,i) => (
        <div key={i} style={{
          position:"fixed", left:`${8+i*7}%`, top:`${10+((i*37)%80)}%`,
          width: 2+i%3, height: 2+i%3, borderRadius:"50%",
          background:"rgba(201,168,76,0.5)",
          animation:`twinkle ${2+i*0.4}s ${i*0.5}s ease-in-out infinite`,
      ...
      <main className="relative px-4 max-w-lg mx-auto space-y-4 pb-24" style={{ zIndex:2 }}>
        ...
        {/* ── Tab Content ── */}
        {tab === "undangan" && <UndanganTab teacher={teacher} />}
...
function UndanganTab({ teacher }) {
  ...
  return (
    <div className="tab-slide rounded-3xl p-6 space-y-5" style={{
      ...
      boxShadow:"0 4px 40px rgba(0,0,0,0.3), inset 0 1px 0 rgba(201,168,76,0.08)",
    }}>
      {/* Bismillah */}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label:"Bapak Guru", items:["Jas eksekutif"], color:"rgba(201,168,76,0.08)", border:"rgba(201,168,76,0.2)", text:"#f5e6c0", sub:"rgba(201,168,76,0.7)", emoji:"🤵" },
            { label:"Ibu Guru",   items:["Baju kebaya"],   color:"rgba(245,200,200,0.05)", border:"rgba(245,180,180,0.2)", text:"#fde8e8", sub:"rgba(245,180,180,0.7)", emoji:"👘" },
          ].map((d,i) => (
            <div key={i} className="rounded-2xl p-4 text-center" style={{ background:d.color, border:`1px solid ${d.border}` }}>
              <div style={{ fontSize:"1.8rem", marginBottom:6 }}>{d.emoji}</div>
              <p style={{ color:d.sub, fontSize:"0.72rem", fontWeight:600, marginBottom:8 }}>{d.label}</p>
              {d.items.map((item, j) => (
                <p key={j} style={{ color:d.text, fontSize:"0.72rem", margin:"3px 0", display:"flex", alignItems:"center", gap:5 }}>
                  <span style={{ color:d.sub, fontSize:"0.55rem" }}>▸</span>{item}
                </p>
              ))}
            </div>
          ))}
